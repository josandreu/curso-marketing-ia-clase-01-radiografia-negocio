...
import { Brain, Eye, Ear, Zap, MessageCircle } from 'lucide-react';
  ...
  const quadrants = [
    ...
    { 
      ...
      items: [
        ...
        'Podcasts de salud que dar consejos contradictorios'
      ],
  ...
  return (
    <SlideShell className="flex flex-col justify-center px-8 md:px-16 overflow-hidden">
      <div className="max-w-6xl mx-auto w-full relative z-10 font-body">
        <h2 className="font-heading text-3xl md:text-4xl lg:text-5xl font-800 text-center mb-2 fade-up fade-up-1 text-foreground/80">
          MAPA DE EMPATÍA — <span className="text-primary truncate">RESULTADO</span>
        </h2>
        
        <p className="text-center text-lg text-muted-foreground font-medium mb-12 fade-up fade-up-1">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 flex-1 mb-10 fade-up fade-up-2">
          {quadrants.map((q, i) => (
            <BentoCard 
              key={i} 
              className={`flex flex-col gap-6 p-8 border ${q.bg} ${q.border} shadow-sm group hover:scale-[1.01] transition-transform`}
            >
              <div className="flex items-center gap-4">
                 <div className="w-10 h-10 rounded-lg flex items-center justify-center bg-white shadow-sm">
                    {q.id === 'ef1' && <Brain className="w-5 h-5 text-primary" />}
                    {q.id === 'ef2' && <Eye className="w-5 h-5 text-accent-foreground" />}
                    {q.id === 'ef3' && <Ear className="w-5 h-5 text-amber-800" />}
                    {q.id === 'ef4' && <Zap className="w-5 h-5 text-emerald-800" />}
                 </div>
                 <p className={`font-heading text-base font-900 uppercase tracking-widest ${q.color}`}>
                   {q.label}
                 </p>
              </div>
              <ul className="space-y-3">
                {q.items.map((item, j) => (
                   <li key={j} className="flex gap-3">
                      <span className={`text-xs mt-1.5 opacity-40 ${q.color}`}>●</span>
        ...
        <div className="fade-up fade-up-3">
          <BentoCard accent className="bg-amber-100/30 border-2 border-amber-200/50 flex flex-col md:flex-row gap-4 items-center p-6">
             <div className="flex-shrink-0 bg-white w-14 h-14 rounded-2xl flex items-center justify-center shadow-sm">
                <MessageCircle className="w-8 h-8 text-amber-600" />
             </div>
             <div>
                <p className="font-mono text-xs uppercase font-bold text-amber-900/60 mb-2 tracking-widest">Lenguaje del cliente (Lo que apunta tu IA)</p>
