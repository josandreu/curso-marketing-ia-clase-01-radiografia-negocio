  ...
  return (
    <SlideShell className="flex flex-col justify-center px-8 md:px-16 overflow-hidden">
      <div className="max-w-5xl mx-auto w-full relative z-10 font-body">
        <p className="font-body text-base tracking-[0.2em] uppercase text-primary mb-3 fade-up fade-up-1 text-center">Resultado — Ejemplo real</p>
        ...
        <div className="flex flex-col gap-4 mb-10 fade-up fade-up-3">
          {examples.map((ex, i) => (
            <BentoCard 
              key={i} 
              className={`flex flex-col md:flex-row gap-8 p-8 border ${ex.bg} ${ex.border} ${ex.flex} shadow-sm group hover:scale-[1.01] transition-transform`}
            >
              <span className={`font-mono text-xs tracking-[3px] uppercase flex-shrink-0 pt-1 min-w-[160px] font-900 ${ex.color}`}>
                {ex.label}
              </span>
              <p className={`text-lg md:text-xl leading-relaxed text-foreground/80 font-medium ${ex.textStyle || ''}`}>
                {ex.text}
              </p>
            </BentoCard>
          ))}
        ...
        <div className="fade-up fade-up-4">
           <div className="bg-destructive/10 border-2 border-destructive/20 rounded-2xl p-8 flex items-center gap-6 justify-center">
              <AlertCircle className="w-8 h-8 text-destructive flex-shrink-0" />
              <p className="text-lg font-bold text-destructive tracking-tight">
                 Si suena a lo que podría decir CUALQUIER empresa de tu sector... <span className="underline underline-offset-8 decoration-destructive/30">BORRA Y REPITE.</span>
              </p>
           </div>
        </div>
      </div>
